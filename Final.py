        ...
        if API_KEY.strip() == '':
            ...
            try:
                ...
                if translate_bengali:
                    ...
                    song_bn = model.generate_content(["Suggest a Bengali song related to this image", img])
                    st.image(img, caption=f"Bengali Caption: {caption_bn.text}")
                    st.write(f"Tags: {tags_bn.text}")
                    st.write(f"Suggested Bengali Song: {song_bn.text}")
                    ...
                    song_hi = model.generate_content(["Suggest a Hindi song related to this image", img])
                    st.image(img, caption=f"Hindi Caption: {caption_hi.text}")
                    st.write(f"Tags: {tags_hi.text}")
                    st.write(f"Suggested Hindi Song: {song_hi.text}")
                else:
                    ...
                    song = model.generate_content(["Suggest a song related to this image", img])
                    st.image(img, caption=f"Caption: {caption.text}")
                    st.write(f"Tags: {tags.text}")
                    st.write(f"Suggested Song: {song.text}")
